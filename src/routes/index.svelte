...
<Timer time={time} text={isNextYear(selectedDeadline) ? "next year" : selectedDeadline}/>

<select id="years" name="Years" bind:value={selectedDeadline} on:change={(ev) => {
    reset()
}}>
    {#each deadlinesIndexed as deadline}
        <option value={deadline} style={
            deadline == selectedDeadline ? `color: aliceblue; background-color: red;` : ""
        }>{deadline}</option>
    {/each} 
</select>
